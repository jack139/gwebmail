...
#define VERSION		"1.005gt"
#define REVDATE		"September 8 1997"
...
char *inbuf;
{
	...
	if (strncmp(inbuf,"user",4) == 0) {
		...
		if( ptr = index(inbuf,'@') ) {
			...
			strncpy(cli_user,inbuf,CLI_BUFSIZ);
		}
		...
		state = SVR_PASS_STATE;
 		...
 		if (verify_user_apop(cli_user,inbuf) == -1) {
 			...
 			/* DTS added 14Feb96 for syslogging */
 			syslog( SYSLOGPRI,
 				"Invalid user or password for \"%s\"",cli_user);
 			return(SVR_AUTH_STATE);
 		}
#ifdef VIRTUAL
		if(virtual_mode) {
			syslog( SYSLOGPRI,
				"Virtual svr_hostname, servicing request for %s",
				cli_user);
			snprintf(svr_buf,SVR_BUFSIZ,"%s%s",virt_spooldir,cli_user);
		} else {
			syslog( SYSLOGPRI,
				"Servicing request for %s",cli_user);
#  ifdef QMAIL
...
#else
 		syslog( SYSLOGPRI, "Servicing request for %s",cli_user);
#  ifdef QMAIL
		pwd=getpwnam(cli_user);
		snprintf(svr_buf,SVR_BUFSIZ,"%s/Mailbox",pwd->pw_dir);
#  else

...
/**************************************************************************
 * prevent to delete temporary directry /usr/tmp/.pop
 */
int my_unlink(const char *pathname)
{
  struct stat s1, s2;
  
  if (stat(DEF_POP3_DIR, &s1)==-1) return -1;
  if (stat(pathname, &s2)==-1) return -1;
...
char *fname;
{
	...
	if ((cnt = fld_select(cli_mbox)) < 0) {
		if( cnt == -2 )
			sprintf(svr_buf,"-ERR being read already %s\r\n",
				cli_mbox);
		else
			sprintf(svr_buf,
...
#  else
		strncpy(fld_fname, DEF_POP3_DIR, FNAME_SIZ);
#  endif
	}
...
fld_release()
{
	...
	if (zap_orig_mbox == REMOVE_MBOX) {
		 ...
		 *	(checking for prior existance for security reasons).
		 */
		strncpy(temp_fname, fld_fname, FNAME_SIZ);
		strncat(temp_fname, ".tmp", FNAME_SIZ-strlen(temp_fname));
